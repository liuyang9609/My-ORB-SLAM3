...
Verbose::eLevel Verbose::th = Verbose::VERBOSITY_NORMAL;    //输出日志类型（详细程度）

System::System(const string &strVocFile,        //字典路径
               const string &strSettingsFile,   //配置文件路径
               const eSensor sensor,            //传感器类型
               const bool bUseViewer,           //可视化viewer开关
               const int initFr,                //初始化帧ID，初值=0
               const string &strSequence):      //序列路径
    //初始化成员列表
    mSensor(sensor),                            //传感器输入类型
    mpViewer(static_cast<Viewer*>(NULL)),       //位姿可视化viewer
    mbReset(false),                             //Reset
    mbResetActiveMap(false),                    //复位激活的地图
    mbActivateLocalizationMode(false),          //纯定位模式，局部建图关闭
    mbDeactivateLocalizationMode(false),        //关闭纯定位模式，开启局部建图
    mbShutDown(false)                           //ShutDown
{
    ...
    cv::FileNode node = fsSettings["File.version"]; //读取YAML文件版本
    if(!node.empty() && node.isString() && node.string() == "1.0"){
        settings_ = new Settings(strSettingsFile,mSensor);

        mStrLoadAtlasFromFile = settings_->atlasLoadFile(); //从文件加载地图集 的路径
        mStrSaveAtlasToFile = settings_->atlasSaveFile();   //将地图集保存至文件 的路径
    ...
    if(!node.empty())   //如果配置文件有设定，则依据配置文件
    {
        activeLC = static_cast<int>(fsSettings["loopClosing"]) != 0;
    }

    mStrVocabularyFilePath = strVocFile;    //copy字典文件路径

    bool loadedAtlas = false;   //是否已经加载地图集标志位，默认为未加载

    if(mStrLoadAtlasFromFile.empty())   //如果不需要从文件加载地图集
    {
        //Load ORB Vocabulary
        //加载ORB字典
        cout << endl << "Loading ORB Vocabulary. This could take a while..." << endl;

        mpVocabulary = new ORBVocabulary(); //创建新的ORB字典
        bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile); //字典成功加载标志位，加载字典文件
        if(!bVocLoad)   //加载字典报错
        {
        ...
        mpAtlas = new Atlas(0); //创建新地图集，初始化关键帧id为0
    }
    else    //需要从文件加载地图集
    {
        //Load ORB Vocabulary
        ...
        bool isRead = LoadAtlas(FileType::BINARY_FILE); //加载地图集
        ...
        loadedAtlas = true; //变更已加载地图集标志位

        mpAtlas->CreateNewMap();    //创建新地图
    ...
    mpLocalMapper->mInitFr = initFr;  //赋值初始化关键帧ID
    if(settings_)   //设置远点和近点的阈值（TUM配置文件里没有，在Tracking构造函数里读取Stereo.ThDepth进行设置）
    ...
    if(mpLocalMapper->mThFarPoints!=0)
    {
        ...
        mpLocalMapper->mbFarPoints = true;  //远点近点阈值设置标志位
    }
    else
...
{
    if(mSensor!=STEREO && mSensor!=IMU_STEREO)  //判断传感器类型是否正确
    {
    ...
    if(settings_ && settings_->needToRectify()){    //如需要矫正且有配置文件
    ...
    else if(settings_ && settings_->needToResize()){    //如需要裁剪且有配置文件
    ...
    //监控模式改变
    {
        ...
        if(mbActivateLocalizationMode)  //纯定位模式，停止局部建图
        {
            mpLocalMapper->RequestStop();   //局部建图停止请求

            // Wait until Local Mapping has effectively stopped
            while(!mpLocalMapper->isStopped())  //等待停止
            {
                usleep(1000);
            }

            mpTracker->InformOnlyTracking(true);    //通知Tracking线程只跟踪不建图
            mbActivateLocalizationMode = false; //设置对应标志位
        }
        if(mbDeactivateLocalizationMode)    //开启局部建图模式，关闭纯定位
        {
    ...
    if (mSensor == System::IMU_STEREO)  //如果是IMU双目要加载IMU数据进来
    ...
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;  //记录当前帧地图点
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;    //记录当前帧关键点
...
void System::Shutdown()
{
    ...
    if(!mStrSaveAtlasToFile.empty())    //保存地图文件
    {
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    if(mSensor==MONOCULAR)  //单目不可用，因为？
    {
        cerr << "ERROR: SaveTrajectoryTUM cannot be used for monocular." << endl;
        return;
    }

    vector<KeyFrame*> vpKFs = mpAtlas->GetAllKeyFrames();   //载入所有关键帧
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);  //根据关键帧ID进行排序

    // Transform all keyframes so that the first keyframe is at the origin.
    // After a loop closure the first keyframe might not be at the origin.
    Sophus::SE3f Two = vpKFs[0]->GetPoseInverse();  //第一帧的位姿

    ofstream f;
    f.open(filename.c_str());   //打开位姿存储文件
    f << fixed;
    ...
    //帧位姿将与其参考关键帧关联起来存储，跟踪失败的关键帧将不会被保存
    //每一帧的数据结构包括：参考关键帧lRit，时间戳lT，跟踪失败标志位lbL
    list<ORB_SLAM3::KeyFrame*>::iterator lRit = mpTracker->mlpReferences.begin();   //参考关键帧lRit
    list<double>::iterator lT = mpTracker->mlFrameTimes.begin();                    //时间戳lT
    list<bool>::iterator lbL = mpTracker->mlbLost.begin();                          //跟踪失败标志位lbL

    for(list<Sophus::SE3f>::iterator lit=mpTracker->mlRelativeFramePoses.begin(),   //赋初值：从开始
        lend=mpTracker->mlRelativeFramePoses.end();                                 //赋初值：结尾
        lit!=lend;                                                                  //判断条件：是否循环到结尾？
        lit++, lRit++, lT++, lbL++)                                                 //循环自加1
    {
        if(*lbL)    //如果跟踪失败跳出循环
            continue;
        ...
        while(pKF->isBad()) //如果是坏帧（被剔除等原因）
        {
            Trw = Trw * pKF->mTcp;  //位姿为
            pKF = pKF->GetParent(); //参考帧为父帧
        }

        Trw = Trw * pKF->GetPose() * Two;   //参考关键帧位姿

        Sophus::SE3f Tcw = (*lit) * Trw;    //当前关键帧位姿：Tcw=Tcr*Trw  当前关键帧=参考帧到当前帧×参考帧位姿
        Sophus::SE3f Twc = Tcw.inverse();   //~(Tcw) => Twc

        Eigen::Vector3f twc = Twc.translation();        //将平移从欧氏群转换成平移向量
        Eigen::Quaternionf q = Twc.unit_quaternion();   //转换成四元数
